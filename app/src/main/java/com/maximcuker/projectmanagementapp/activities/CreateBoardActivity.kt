...
class CreateBoardActivity : BaseActivity() {
    ...
    private fun uploadBoardImage() {
        ...
        if(mSelectedImageFileURI != null) {
            ...
            sRef.putFile(mSelectedImageFileURI!!).addOnSuccessListener {
                    taskSnapshot-> Log.i("Board image URL", taskSnapshot.metadata?.reference?.downloadUrl.toString())
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == Constants.PICK_IMAGE_REQUEST_CODE && data?.data != null) {
            ...
            try {
                Glide.with(this).load(mSelectedImageFileURI).centerCrop().placeholder(R.drawable.ic_board_place_holder).into(iv_board_image)
            } catch (e: IOException) {
                e.printStackTrace()
            }
        }
